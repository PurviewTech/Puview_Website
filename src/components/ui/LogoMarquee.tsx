...
export const LogoMarquee = ({
  ...
  // Duplicate once -> we should animate only -50% for seamless loop
  const duplicatedLogos = [...logos, ...logos];
  const duration = `${Math.max(12, Number(speed) || 30)}s`;

  return (
    <div className={cn("overflow-hidden relative w-full py-8", className)}>
      {/* ✅ Keyframes scoped locally so it always works */}
      <style>{`
        @keyframes pm_marquee {
          0% { transform: translateX(0); }
          100% { transform: translateX(-50%); }
        }
        @keyframes pm_marquee_reverse {
      ...
      <div
        ...
        style={{
          animation: `${reverse ? "pm_marquee_reverse" : "pm_marquee"} ${duration} linear infinite`,
